...
@register_model("ESM2_classification")
class ESMEGNNModel(TransformerModel):
    @staticmethod
    def add_args(parser):
        """
    ...
    def __init__(self, args, encoder, decoder):
        ...
        self.mapping = nn.Linear(args.encoder_embed_dim, 2)
    ...
    def forward(self, coor_features,
        aa_features,
        src_lengths,
        prev_output_tokens,
        tgt_lengths,
        ...
        results = self.encoder(ground_truth, repr_layers)
        hiddens = results["representations"][self.encoder_layers]  # [batch, length, dim]
        feature = torch.max(hiddens, dim=1)[0]
        prob = self.mapping(feature)
        # encoder_prob = F.log_softmax(prob, dim=-1)
        encoder_prob = F.softmax(prob, dim=-1)
        return encoder_prob
...
@register_model_architecture("ESM2_classification", "ESM2_classification")
def base_architecture(args):
    transformer_base_architecture(args)


@register_model_architecture("ESM2_classification", "ESM2_classification_base")
...
@register_model_architecture("ESM2_classification", "ESM2_classification_esm")
