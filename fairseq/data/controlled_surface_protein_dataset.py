...
def collate(
    ...
    def collate_features(value):
        ...
        res = values[0].new(batch_size, size, 4, 3).fill_(0.0)
    ...
    aa_identity_target = collate_aa_identity(
        "aa_identity_feature",
        1,
    )
    ...
    src_lengths = torch.LongTensor(
        [s["coor_feature"].size(0) for s in samples]
    )
    ...
    batch = {
        ...
        "ntokens": ntokens,
        "net_input": {"aa_identity_features": aa_identity_target, "coor_features": coor_features,
...
class ControlledSurfaceProteinDataset(FairseqDataset):
    """
    ...
    def __init__(
        ...
        if tgt is not None:
            assert len(aa_identity) == len(tgt)
        self.aa_identity = aa_identity
        self.coor = coors
        self.tgt = tgt
        self.aa_identity_sizes = np.array(aa_identity_sizes)
        self.coor_sizes = np.array(coors_sizes)
        self.tgt_sizes = np.array(tgt_sizes) if tgt_sizes is not None else None
        self.sizes = (
            np.vstack((self.coor_sizes, self.tgt_sizes)).T
            if self.tgt_sizes is not None
            else self.coor_sizes
        )
    ...
    def __getitem__(self, index):
        aa_identity_item = self.aa_identity[index]
        coor_item = self.coor[index]
        tgt_item = self.tgt[index]

        example = {
            "id": index,
            "aa_identity_feature": aa_identity_item,
    ...
    def num_tokens_vec(self, indices):
        ...
        sizes = self.aa_identity_sizes[indices]
        if self.tgt_sizes is not None:
    ...
    def ordered_indices(self):
        ...
        if self.buckets is None:
            ...
            return indices[np.argsort(self.aa_identity_sizes[indices], kind="mergesort")]
        else:
    ...
    def filter_indices_by_size(self, indices, max_sizes):
        ...
        return data_utils.filter_paired_dataset_indices_by_size(
            self.aa_identity_sizes, self.tgt_sizes, indices, max_sizes,
        )
