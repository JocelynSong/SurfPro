...
class IterativeRefinementGenerator(object):
    ...
    def generate(self, models, sample, prefix_tokens=None, constraints=None):
        ...
        src_tokens = sample["net_input"]["atom_features"]
        src_lengths = sample["net_input"]["src_lengths"]
        bsz, src_len = src_tokens.size()
        length_tgt = torch.LongTensor(
        [s.size(0) for s in sample["net_input"]["tgt_tokens"]]
    )

        # initialize
        encoder_out = model.forward_encoder(**sample["net_input"])
        prev_decoder_out = model.initialize_output_tokens(length_tgt, src_tokens, encoder_out)
